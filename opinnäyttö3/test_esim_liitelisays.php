...
if(isset($_POST['nappi'])) {
  ...
  $kysely = $yhteys->prepare("INSERT INTO 707A_toimielimet_poytakirjasisalto (toimielin, osio_nimi, pkid, vuosi) VALUES (?, ?, ?, ?)");
  $kysely->execute(array($toimielin,  $nimi, $nro, $year));
 ...
 if(isset($_POST['tnappi'])) {
   ...
   $kysely = $yhteys->prepare("INSERT INTO 707A_toimielimet_poytakirjasisalto_osallistujat (toimielin, pkid, etunimi, sukunimi, tehtava) VALUES (?, ?, ?, ?, ?)");
   $kysely->execute(array($toimielin, $nro, $enimi, $snimi, $tehtava));
...
<body>
  ...
  <form method="post">
  <p> Lisää osallistuja pöytäkirjaan </p>
      <input type="text" name="etunimi">
      <input type="text" name="sukunimi">
      <input type="text" name="teht">

    <button type="submit" name="tnappi"> Lähetä </button>
